...
void main() {
    ...
    vec3 position_x = reconstruct_view_position(uv_x, depth_x);
    vec3 position_y = reconstruct_view_position(uv_y, depth_y);
    vec3 normal = normalize(cross(position_x - center, position_y - center));
...
//  }
    ...
    for (int direction_index = 0; direction_index < 8; ++direction_index) {
        vec2 screen_direction = directions[direction_index];
        ...
        for (int i = 1; i <= 32; ++i) {
            ...
            float t = float(i) / float(horizon_steps);
            vec2 sample_uv = TexCoord + screen_direction * texel_size * (t * horizon_radius_pixels);
            if (outside(sample_uv)) {
                break;
            }

            float sample_depth = texture(source_depth, sample_uv).r;

            if (sample_depth >= 1) {
                continue;
            }

            vec3 sample_position = reconstruct_view_position(sample_uv, sample_depth);
            vec3 offset = sample_position - center;
            float distance_to_sample = length(offset);

            if (distance_to_sample <= 0) {
                continue;
            }

            vec3 direction = offset / distance_to_sample;
            float elevation = asin(clamp(dot(normal, direction), -1.0, 1.0));
            horizon = max(horizon, elevation);

//          float distance_weight = 1.0 - smoothstep(0.0, horizon_radius_pixels, t * horizon_radius_pixels);
//          horizon = max(horizon, elevation * distance_weight);
            valid_samples += 1.0;
        }

        if (valid_samples > 0.0) {
            float bias_angle = asin(clamp(horizon_bias, -1.0, 1.0));
            float directional_occlusion = max(horizon - bias_angle, 0.0);
//          float directional_occlusion = max(horizon - horizon_bias, 0.0);
    ...
    float ao = clamp(1.0 - ao_strength * occlusion, 0.0, 1.0);
